...
public class RouletteSelectionParentReplacementGeneticAlgorithm implements GeneticAlgorihtm {
    ...
    public Population cross(float percentage) {
        ...
        float accumulate;
        int pos;
        TestContainer sonEvaluation;
        float sonEvaluationFitness;
        ...
        for(int i = 0; i < numberOfCross; i++){
            ...
            for(int j = 0; j < sons.length; j ++){
            	...
            	sonEvaluationFitness = sonEvaluation.getFitness();
                // parents remplacement 
                if(parents[i][0]!=0){
                	if((nextGeneration[parents[i][0]] == null)||(isWorse(nextValues[parents[i][0]].getFitness(), sonEvaluationFitness))){
                		nextGeneration[parents[i][0]] = sons[j];
                		nextValues[parents[i][0]]= sonEvaluation;
                		nextValues[parents[i][0]].setFitness(sonEvaluationFitness);
                	}else if(parents[i][1]!=0){
                		if((nextGeneration[parents[i][1]] == null)||(isWorse(nextValues[parents[i][1]].getFitness(), sonEvaluationFitness))){
                			nextGeneration[parents[i][1]] = sons[j];
                			nextValues[parents[i][1]]= sonEvaluation;
                			nextValues[parents[i][1]].setFitness(sonEvaluationFitness);
                		}
                	}
