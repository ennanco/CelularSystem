...
public class AdvanceOperon implements Operon {
	private boolean constitutive;
	private float[] activationPercentage;
	private Gene[] genes;
	private String[] promoter;
	
	public AdvanceOperon(boolean constitutive, String[] promoter, float[] activationPercentage, Gene[] genes) {
	...
	public String[] active(ProteinList list) throws InternalErrorException {
        ...
        for( int i = 0; i < promoter.length; i++){
            ...
            for(Iterator iter = list.listedProteins().iterator(); iter.hasNext()&&(!activate);){
                ...
                activate = /*nonMatches>0?false:(activationPercentage[i] <concentration);*/((nonMatches + 1) * activationPercentage[i]) < concentration ;
                if(activate){
	...
	public String toString(){
		...
		toReturn+="Length: "+genes.length +"\n";
			for (int i = 0; i <promoter.length;i++){
				toReturn+= "Promoter: " + promoter[i] + " Percentage: " + activationPercentage[i] + "\n";
			}
